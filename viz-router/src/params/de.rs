...
mod tests {
    ...
    fn test_parse_single_value() {
        ...
        assert_eq!(
            MyEnum::deserialize(PathDeserializer::new(&params)).unwrap(),
            MyEnum::B
        );

        let params: Params = vec![("a", "1"), ("b", "2")].into();
        assert_eq!(
            i32::deserialize(PathDeserializer::new(&params))
                .unwrap_err()
                .to_string(),
    ...
    fn test_request_extract() {
        ...
        let s: () = de::Deserialize::deserialize(PathDeserializer::new(&params)).unwrap();
        assert_eq!(s, ());

        let s: MyStruct = de::Deserialize::deserialize(PathDeserializer::new(&params)).unwrap();
        assert_eq!(s.key, "name");
        assert_eq!(s.value, "user1");

        let s: (String, String) =
            de::Deserialize::deserialize(PathDeserializer::new(&params)).unwrap();
        assert_eq!(s.0, "name");
        assert_eq!(s.1, "user1");

        let s: &str = de::Deserialize::deserialize(PathDeserializer::new(&params)).unwrap();
        assert_eq!(s, "name");

        let params: Params = vec![("key", "name"), ("value", "32")].into();

        let s: Test1 = de::Deserialize::deserialize(PathDeserializer::new(&params)).unwrap();
        assert_eq!(s.0, "name");
        assert_eq!(s.1, 32);

        #[derive(Deserialize)]
        struct T(Test1);
        let s: T = de::Deserialize::deserialize(PathDeserializer::new(&params)).unwrap();
        assert_eq!((s.0).0, "name");
        assert_eq!((s.0).1, 32);

        let s: Test2 = de::Deserialize::deserialize(PathDeserializer::new(&params)).unwrap();
        assert_eq!(s.key, "name");
        assert_eq!(s.value, 32);

        let s: Result<(Test2,), _> = de::Deserialize::deserialize(PathDeserializer::new(&params));
        assert!(s.is_err());

        let s: (String, u8) = de::Deserialize::deserialize(PathDeserializer::new(&params)).unwrap();
        assert_eq!(s.0, "name");
        assert_eq!(s.1, 32);
    ...
    fn test_extract_path_single() {
        ...
        let i: i8 = de::Deserialize::deserialize(PathDeserializer::new(&params)).unwrap();
        assert_eq!(i, 32);

        let i: (i8,) = de::Deserialize::deserialize(PathDeserializer::new(&params)).unwrap();
        assert_eq!(i, (32,));

        let i: Result<(i8, i8), _> = de::Deserialize::deserialize(PathDeserializer::new(&params));
        assert!(i.is_err());

        #[derive(Deserialize)]
        struct Test(i8);
        let i: Test = de::Deserialize::deserialize(PathDeserializer::new(&params)).unwrap();
        assert_eq!(i.0, 32);

        let params: Params = vec![("value", "32"), ("value2", "32")].into();
        let i: Result<i8, _> = de::Deserialize::deserialize(PathDeserializer::new(&params));
        assert!(i.is_err());
    }
    ...
    fn test_extract_enum() {
        ...
        let i: TestEnum = de::Deserialize::deserialize(PathDeserializer::new(&params)).unwrap();
    ...
    fn test_extract_enum_value() {
        let params: Params = vec![("val", "val1")].into();
        let i: Test3 = de::Deserialize::deserialize(PathDeserializer::new(&params)).unwrap();
